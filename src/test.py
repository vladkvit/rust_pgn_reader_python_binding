...
13. b3 { [%eval -4.14] [%clk 0:00:02] } 13... Nf4 { [%eval -2.73] [%clk 0:00:21] } 0-1"""
    ...
    def test_multithreaded_correctness(self):
        """Test that multithreaded parsing produces identical results."""
        # Generate enough games to spread across multiple threads
        pgns = []
        for i in range(40):
            if i % 3 == 0:
        ...
        # position_to_game agrees between thread counts
        ...
        # move_to_game agrees between thread counts
        ...
        # Global flat arrays are identical regardless of thread count
        np.testing.assert_array_equal(result_1t.boards, result_4t.boards)
        np.testing.assert_array_equal(result_1t.from_squares, result_4t.from_squares)
        np.testing.assert_array_equal(result_1t.move_offsets, result_4t.move_offsets)
        np.testing.assert_array_equal(
            result_1t.position_offsets, result_4t.position_offsets
        )
    ...
    def test_flat_array_getters(self):
        """Test ParsedGames exposes correct global flat arrays."""
        ...
        # Scalar getters
        self.assertEqual(result.num_games, 2)
        self.assertEqual(result.num_moves, 5)
        self.assertEqual(result.num_positions, 7)

        # Array shapes
        self.assertEqual(result.boards.shape, (7, 8, 8))
        self.assertEqual(result.castling.shape, (7, 4))
        self.assertEqual(result.en_passant.shape, (7,))
        self.assertEqual(result.halfmove_clock.shape, (7,))
        self.assertEqual(result.turn.shape, (7,))
        self.assertEqual(result.from_squares.shape, (5,))
        self.assertEqual(result.to_squares.shape, (5,))
        self.assertEqual(result.promotions.shape, (5,))
        self.assertEqual(result.clocks.shape, (5,))
        self.assertEqual(result.evals.shape, (5,))
        self.assertEqual(result.is_checkmate.shape, (2,))
        self.assertEqual(result.is_stalemate.shape, (2,))
        self.assertEqual(result.is_insufficient.shape, (2, 2))
        self.assertEqual(result.legal_move_count.shape, (2,))
        self.assertEqual(result.valid.shape, (2,))

        # CSR offset shapes and values
        self.assertEqual(result.move_offsets.shape, (3,))  # 2 games + 1
        self.assertEqual(result.position_offsets.shape, (3,))
        np.testing.assert_array_equal(result.move_offsets, [0, 2, 5])
        np.testing.assert_array_equal(result.position_offsets, [0, 3, 7])
        self.assertEqual(result.parsed_move_counts.shape, (2,))
        np.testing.assert_array_equal(result.parsed_move_counts, [2, 3])

        # List getters
        self.assertEqual(len(result.headers), 2)
        self.assertEqual(len(result.outcome), 2)
        self.assertEqual(result.outcome[0], "White")
        self.assertEqual(result.outcome[1], "Black")
        self.assertEqual(result.parse_errors, [None, None])

        # Flat array values match per-game views
        game0 = result[0]
        game1 = result[1]
        np.testing.assert_array_equal(game0.boards, result.boards[:3])
        np.testing.assert_array_equal(game1.boards, result.boards[3:])
    ...
    def test_repr(self):
        """Test __repr__ on ParsedGames and PyGameView."""
        pgns = ["1. e4 e5 1-0"]
        result = rust_pgn_reader_python_binding.parse_games_from_strings(pgns)
