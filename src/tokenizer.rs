...
//! (e.g. with `shakmaty::san::SanPlus::from_ascii`). This keeps lexical
//! scanning cheap and reusable (e.g. a counting or boundary-detection pass
//! needs no SAN parsing).
//!
//! Known intentional divergences from pgn-reader (the previous parser):
...
/// Where parsing of one game stopped, and why.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct GameEnd {
    /// Bytes consumed by the game (terminating construct excluded except
    /// for result tokens and `*`, which are part of the game).
    pub consumed: usize,
    /// `true` when an explicit game terminator (result token, `*`, blank
    /// line, or start of a new tag section) was found; `false` when the
    /// input was exhausted first (i.e. the game may be truncated and
    /// `consumed == input.len()`).
    pub terminated: bool,
}
...
/// Flush `end_headers` exactly once, when the first movetext-ish token
/// arrives. Every match arm that can start the movetext calls this.
#[inline]
fn leave_headers<V: Visitor>(in_headers: &mut bool, visitor: &mut V) {
    if std::mem::replace(in_headers, false) {
        visitor.end_headers();
    }
}
...
///
/// Contract: one game per input. Parsing stops at the first game
/// terminator: a result token, `*`, a blank line, or the start of a new
/// tag section (parity with a single `pgn_reader::Reader::read_game`
...
pub fn parse_game<V: Visitor>(mut bytes: &[u8], visitor: &mut V) -> GameEnd {
    ...
    while cursor < len {
        ...
        match c {
            ...
            b'{' => {
                leave_headers(&mut in_headers, visitor);
                cursor += 1;
                ...
                if let Some(end_offset) = memchr(b'}', &bytes[cursor..]) {
                    ...
                    // No closing brace, consume rest of input
                    if variation_depth == 0 {
...
mod tests {
    ...
    fn test_castling_with_zeros_delivered_raw() {
        ...
        // the tokenizer delivers the tokens verbatim.
    ...
    fn test_trailing_game_ignored_after_result() {
        ...
        let end = {
            let mut visitor = RecordingVisitor::default();
            let end = parse_game(pgn.as_bytes(), &mut visitor);
            assert!(visitor.events.contains(&"header Event=a".to_string()));
            assert!(!visitor.events.contains(&"header Event=b".to_string()));
            assert_eq!(
                visitor
                    .events
                    .iter()
                    .filter_map(|e| e.strip_prefix("san "))
                    .count(),
                1
            );
            assert!(visitor.events.contains(&"outcome WhiteWins".to_string()));
            assert!(!visitor.events.contains(&"outcome BlackWins".to_string()));
            end
        };
        // Terminated at the result token; the second game is unconsumed.
        assert!(end.terminated);
        assert_eq!(&pgn[end.consumed..], "\n\n[Event \"b\"]\n\n1. d4 0-1");
    }
    ...
    #[test]
    fn test_game_end_reporting() {
        // Explicit terminator: result token, terminated.
        let mut v = RecordingVisitor::default();
        let end = parse_game(b"1. e4 e5 1-0 trailing", &mut v);
        assert!(end.terminated);
        assert_eq!(end.consumed, "1. e4 e5 1-0".len());

        // EOF: no terminator in sight.
        let mut v = RecordingVisitor::default();
        let end = parse_game(b"1. e4 e5 2.", &mut v);
        assert!(!end.terminated);
        assert_eq!(end.consumed, "1. e4 e5 2.".len());
    }
}
